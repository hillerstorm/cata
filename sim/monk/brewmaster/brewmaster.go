...
func RegisterBrewmasterMonk() {
	core.RegisterAgentFactory(
		...
		func(player *proto.Player, spec interface{}) {
			playerSpec, ok := spec.(*proto.Player_BrewmasterMonk) // I don't really understand this line
			if !ok {
...
func (bm *BrewmasterMonk) SetStartingStance() {
	if bm.StartingStance == proto.MonkStance_SturdyOx {
		bm.Monk.Stance = monk.SturdyOx
	} else {
		bm.Monk.Stance = monk.FierceTiger
	}
}
...
func (bm *BrewmasterMonk) ApplyTalents() {
	...
	bm.ApplyArmorSpecializationEffect(stats.Stamina, proto.ArmorType_ArmorTypeLeather)
}
