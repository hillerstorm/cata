...
func init() {
	...
	for _, v := range []shared.ItemVersion{shared.ItemVersionLFR, shared.ItemVersionNormal, shared.ItemVersionHeroic} {
		version := v // Gotta scope this for the closure
		labelSuffix := version.GetLabel()
...
type TentacleOfTheOldOnesPet struct {
	core.Pet
	mindFlay           map[shared.ItemVersion]*core.Spell
	ProccedItemVersion shared.ItemVersion
}
...
func (pet *TentacleOfTheOldOnesPet) registerMindFlay() {
	...
	pet.mindFlay = make(map[shared.ItemVersion]*core.Spell)
	for _, v := range []shared.ItemVersion{shared.ItemVersionLFR, shared.ItemVersionNormal, shared.ItemVersionHeroic} {
		version := v // Gotta scope this for the closure
