...
func (unit *Unit) EnableEnergyBar(maxEnergy float64, maxComboPoints int32, unitClass proto.Class) {
	unit.SetCurrentPowerBar(EnergyBar)

	unit.energyBar = energyBar{
		unit:                    unit,
		maxEnergy:               max(100, maxEnergy),
		maxComboPoints:          max(5, maxComboPoints),
...
func (eb *energyBar) AddChi(sim *Simulation, pointsToAdd int32, metrics *ResourceMetrics) {
	eb.AddComboPoints(sim, pointsToAdd, metrics)
}
